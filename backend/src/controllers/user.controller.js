...
exports.updateMe = asyncHandler(async (req, res, _next) => {
    ...
    if (name !== undefined || monthlyBudget !== undefined || currency !== undefined || avatarUrl !== undefined || address !== undefined || onboardingCompleted !== undefined) {
        await userService.updateProfile(req.user._id, { name, monthlyBudget, currency, avatarUrl, address, onboardingCompleted });
        await cacheService.del(cacheService.generateUserKey(req.user._id, 'profile'));
    }

    // Update household if provided
    if (household !== undefined) {
        await userService.updateHousehold(req.user._id, household);
        await cacheService.del(cacheService.generateUserKey(req.user._id, 'profile'));
    }

    // Update preferences if provided
    if (planPreferences !== undefined) {
        await userService.updatePreferences(req.user._id, planPreferences);
        await cacheService.del(cacheService.generateUserKey(req.user._id, 'profile'));
    }

    // Update activePlan if provided (null = delete plan)
    ...
    if (streak !== undefined && lastCheckIn !== undefined) {
        await userService.updateStreak(req.user._id, streak, new Date(lastCheckIn));
    ...
    sendSuccess(res, 200, 'Profile updated.', { updated: true, updatedAt: new Date() });
});
...
exports.getActivePlan = asyncHandler(async (req, res, _next) => {
    ...
    sendSuccess(res, 200, activePlan ? 'Active plan fetched.' : 'No active plan.', activePlan);
});
...
exports.updateHeatmap = asyncHandler(async (req, res, _next) => {
    ...
    const result = await userService.recordHeatmapEntry(req.user._id, completedCount, totalCount);

    // Bust the per-month heatmap cache
    const now = new Date();
    const monthKey = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, '0')}`;
    await cacheService.del(cacheService.generateUserKey(req.user._id, `heatmap:${monthKey}`));
...
exports.getHeatmap = asyncHandler(async (req, res, _next) => {
    ...
    const cacheKey = cacheService.generateUserKey(req.user._id, `heatmap:${monthKey}`);

    let heatmapData = await cacheService.get(cacheKey);

    if (!heatmapData) {
        heatmapData = await userService.getMonthlyHeatmap(req.user._id, year, month);
...
exports.deleteAccount = asyncHandler(async (req, res, _next) => {
    ...
    await cacheService.delPattern(cacheService.generateUserKey(req.user._id, '*'));
...
exports.updateUserStatus = asyncHandler(async (req, res, _next) => {
    ...
    await cacheService.delPattern(cacheService.generateUserKey(req.params.id, '*'));
